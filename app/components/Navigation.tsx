    ...
    <nav className="bg-emerald-500 text-white fixed w-full z-50">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-16">
          ...
          >
            <a href="/" className="font-bold text-2xl tracking-tight">Fast-Fol</a>
          </motion.div>

          {/* Desktop Navigation */}
          <div className="hidden md:block">
            <div className="ml-10 flex items-baseline space-x-6">
              {['Główna', 'O Firmie', 'Oferta', 'Kontakt'].map((item, index) => (
                <motion.a
                  key={item}
                  href={item === 'Główna' ? '/' : `/${item.toLowerCase().replace(' ', '-')}`}
        ...
        >
          <div className="px-2 pt-2 pb-3 space-y-1 sm:px-3">
            {['Główna', 'O Firmie', 'Oferta', 'Kontakt'].map((item) => (
              <a
                key={item}
                href={item === 'Główna' ? '/' : `/${item.toLowerCase().replace(' ', '-')}`}
